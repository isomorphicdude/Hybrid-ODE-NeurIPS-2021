...
def make_y_net(input_size,
    ...
    #NOTE: if on other data, replace with the same structure as make_w_net
...
#TODO: modify to incoporate expert knowledge
...
class SDENet(torchsde.SDEStratonovich):
    def __init__(self,
        ...
        # assign initial params to a nn.Parameter
        ...
        # Final projection layer (Used in classification)
        self.projection = nn.Sequential(
            nn.Flatten(),
    ...
    def f(self, t, y: torch.Tensor):
        ...
        y, w, _ = y.split(split_size=(y.numel() - self.params_size - 1, 
                                      self.params_size, 
                                      1), 
                          dim=1) # params_size: 606408
        
        # drift
        fy = self.y_net(t, 
                        y.reshape((-1, *self.aug_input_size)), 
                        self.unravel_params(w)).reshape(-1)
        nn = self.w_net(t, w)
    ...
    def forward(self, y, 
                adjoint=False, dt=0.02, 
                adaptive=False, adjoint_adaptive=False, 
                method="midpoint", 
                rtol=1e-4, atol=1e-3):
        ...
        aug_y = torch.cat((y.reshape(-1), 
                           ...
                           torch.tensor([0.], device=y.device))) # 841609: (235200, 606408, 1)
        aug_y = aug_y[None]
        ...
        y1 = aug_y1[:y.numel()].reshape(y.size()) # the last bit is removed
        
        # Projection layer.
        logits = self.projection(y1)
        logqp = .5 * aug_y1[-1] # KL divergence
        return logits, logqp
